...
plugin = Plugin()
addon = xbmcaddon.Addon("plugin.video.filmhpo.playlist")
pluginrootpath = "plugin://plugin.video.filmhpo.playlist"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.filmhpo.playlist/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.filmhpo.playlist(/.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.filmhpo.playlist" in item["path"]:
